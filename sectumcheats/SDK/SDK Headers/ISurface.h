...
public:
	...
	void DrawLine1(int x0, int y0, int x1, int y1, CColor Color)
	{
		typedef void(__thiscall* OriginalFn)(PVOID, int, int, int, int);
		getvfunc< OriginalFn >(this, 19)(this, x0, y0, x1, y1);
	}
	void RenderLine(int x, int y, int x2, int y2, CColor color)
	{
	...
	void DrawStringY(int font, int x, int y, CColor color, DWORD alignment, const char* msg, ...)
	{
		va_list va_alist;
		char buf[1024];
