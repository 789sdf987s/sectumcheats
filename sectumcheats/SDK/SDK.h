...
typedef matrix3x4_t matrix3x4_ch;
...
public:
	void InitialiseSDK()
	{
		auto p = Utils.PatternSearch("client_panorama.dll", (BYTE*)"\x8B\x0D\x00\x00\x00\x00\xFF\x75\x08\x8B\x01\xFF\x50\x64", "xx????xxxxxxxx", NULL, NULL);
		if (p)
		{
		...
		g_pGlowObjectManager = *(DWORD *)(Utils.PFindPattern("client_panorama.dll", "0F 11 05 ?? ?? ?? ?? 83 C8 01 C7 05 ?? ?? ?? ?? 00 00 00 00") + 3);
		...
		g_pViewRenderBeams = *(IViewRenderBeams**)(Utils.PFindPattern("client_panorama.dll", "B9 ?? ?? ?? ?? A1 ?? ?? ?? ?? FF 10 A1 ?? ?? ?? ?? B9") + 1);
		cout << white << "g_pViewRenderBeams" << ": " << igreen << "0x" << g_pViewRenderBeams << white << endl;



		OffsetLineGoes = (DWORD)Utils.PatternSearch("client_panorama.dll", (PBYTE)"\x55\x8B\xEC\x83\xEC\x08\x8B\x15\x00\x00\x00\x00\x0F\x57\xC0", "xxxxxxxx????xxx", NULL, NULL);
